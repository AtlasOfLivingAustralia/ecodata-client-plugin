...
(function () {
    ...
    ecodata.forms.Scores = function(scoresData, config) {
        var self = this;
        self.scores = {};
        _.each(scoresData, function(scoreData) {
            if (self.scores[scoreData.scoreId]) {
                self.scores[scoreData.scoreId].update()
            }
            else {
                self.scores[scoreData.scoreId] = new ScoreAndTarget(scoreData)
            }
        });


        self.update = function() {
            var data = master.modelAsJs();
            $.ajax({
                method:'POST',
                url: config.updateScoresUrl,
                data:data
            })
        }
    }

    var ScoreAndTarget = function(scoreData) {

        var self = this;
        if (!scoreData) {
            scoreData = {};
        }
        self.scoreId = scoreData.scoreId;
        self.value = ko.observable();
        self.annualTarget = scoreData.annualTarget;
        self.overallTarget = scoreData.overallTarget;


        self.update = function(scoreData) {
            var currentScore = scoreData.result && scoreData.result.result;
            self.value(currentScore);
            self.target = scoreData.target;
            self.annualTarget = scoreData.overallTarget;
        }

        self.warningThresholdExceeded = ko.computed(function() {
            return self.value() > self.overallTarget * 2;
        });

        self.register = function(dataModelItem) {
            dataModelItem.subscribe(function() {
                self.update();
            });
        };

        self.update(scoreData);
    }
}());
